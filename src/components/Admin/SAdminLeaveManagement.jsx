...
const SAdminLeaveManagement = () => {
  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const response = await axios.get(
        "https://springbootprojecterp.up.railway.app/student/appliedforleave"
      );
  ...
  const handleStatusUpdate = async (leaveId, status) => {
    ...
    try {
      await axios.put(
        `https://springbootprojecterp.up.railway.app/student/update/${leaveId}?status=${status}`
      );
      setLeaveRequests(
                    ...
                    leaveRequests.map((request) => (
                      <tr key={request.leaveId} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          {request.student && request.student.name
...
export default SAdminLeaveManagement;
